# dosya adi: diffusion_sweep_rule.py
# #5 - BER vs diffusion coefficient D (3D Fickian), krec = 0.8*D/d^2 (her D'de yeniden)
# Ciktilar: diffusion_sweep_rule.png , .pdf
...
Q = 50000; D_DIST = 4.0
DIFFUSION_VALUES = [10, 20, 30, 40, 60, 80]; KREC_OLD = 0.45
...
def build_fick_channel(bits, Dc):
    n = int(NUM_BITS*T_BIT/DT); C = np.zeros(n)
    pt = np.maximum(np.arange(0, 2.0, DT), 1e-12)
    p = (Q/((4*np.pi*Dc*pt)**1.5))*np.exp(-(D_DIST**2)/(4*Dc*pt)); p *= 0.005
...
print("D[um^2/s]  krec_rule")
for Dc in DIFFUSION_VALUES:
    kr = 0.8*Dc/(D_DIST**2); print(f"   {Dc:>3}     {kr:.2f}")
    acc = {l: 0.0 for l in labels}
    for _ in range(NUM_ITERATIONS):
        bits = np.random.randint(0, 2, NUM_BITS)
        Cn = np.random.poisson(build_fick_channel(bits, Dc)*SCALE)/SCALE
...
for lab, col in zip(labels[:5], colors): ax.plot(DIFFUSION_VALUES, res[lab], "o-", color=col, label=lab)
ax.plot(DIFFUSION_VALUES, res[labels[5]], "s--", color="0.6", lw=1.8, label=labels[5])
ax.set_xlabel("Diffusion Coefficient $D$ ($\\mu$m$^2$/s)")
ax.set_ylabel("Average Bit Error Rate (BER)")
ax.set_title("BER versus Diffusion Coefficient under 3D Fickian Diffusion Channel")
ax.grid(True, which="both", ls="--", alpha=0.4); ax.legend(fontsize=9, loc="best")
plt.tight_layout()
plt.savefig("diffusion_sweep_rule.png", dpi=200)
plt.savefig("diffusion_sweep_rule.pdf")
plt.show()
